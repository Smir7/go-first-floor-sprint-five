...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	// ваш код здесь
	caloriesRun := ((runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return caloriesRun
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	result := math.Pow(speed, 2)
	calories := ((walkingCaloriesWeightMultiplier*weight + (result/heightInCm)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
